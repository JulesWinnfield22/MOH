...
const filteredRowsApproved = computed(() => {
  return contract.contracts.filter(
    (row) => row.contractStatus === 'Approved',
    'submitted',
    'Declined'
  );
});
...
const currentRows = computed(() => {
  ...
  if (selectedTable.value === 'Submitted') return filteredRowsSubmitted.value;
...
<template>
  <div class="bg-[#FBFBFB] p-5 overflow-x-scroll show-scrollbar">
    <div v-if="isRoleHrdi" class="flex justify-between items-center">
      ...
      <div class="p-4 gap-2.5 flex">
        ...
        >
          ...
          {{ (contract.contracts || []).length }} contracts
        </p>
    ...
    <div v-if="selectedTable === 'Submitted'">
      Submitted
      <Table
        :Fallback="TableRowSkeleton"
        :firstCol="isRoleHrdi"
