...
const program = ref([])
 watch(selectedRow, () => {
  console.log((paginations.data.value || []).find(el => el.universityName == selectedRow.value?.universityName))
  program.value = (paginations.data.value || []).find(el => el.universityName == selectedRow.value?.universityName)?.programs || []
}, {deep: true})
</script>
<template>
        ...
        >
          ...
          <div class="col-span-1">
            ...
            <select
              ...
              validation="required"
            >
              <option :key="uni.universityName" :value="uni.universityName" v-for="uni in (paginations.data.value || [])">
                {{ uni.universityName }}
              </option>
            </select>
          </div>
