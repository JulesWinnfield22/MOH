...
import { getStudentsByUniId, getUniStudents, getStudentsByStatusUniId, getStudentsByRegistrationStatus } from '@/features/students/api/studentApi.js';
  ...
  <div class="bg-[#FBFBFB] p-5">
    ...
    <div v-if="isRoleHrdi" class="flex justify-between items-center">
      ...
      <input
        ...
        v-model="pagination.search.value" placeholder="Search Students" />
      <div class="p-4 gap-2.5 flex">
        ...
        <button style="padding: 8px 16px; border-radius: 6px"
    ...
    <div v-if="isRoleHrdi" class="my-10">
      <div v-if="isWithdrawStudent" class="fixed inset-0  flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white rounded-lg shadow-lg gap-3 flex flex-col space-between-[24px] p-6 w-[877px] h-[302px]">
          <div class="flex justify-between">
            ...
            <button class="h-[13px] w-[13px] px-4 py-2 rounded" @click="closeModal">
              <svg width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M15 2L2.00005 14.9999M2 1.99995L14.9999 14.9999" stroke="#FF4040" stroke-width="3.5"
                  stroke-linecap="round" />
              </svg>
            </button>
          </div>
          <select v-model="camStatus" class="border w-[829px] border-[#D9D9D9] bg-[#FBFBFB] rounded p-2 mb-4">
          ...
          <select v-model="reasons" class="border w-[829px] border-[#D9D9D9] bg-[#FBFBFB] rounded p-2 mb-4">
          ...
          <textarea v-if="reasons === 'Other'" v-model="discription" placeholder="State the reason for rejection"
            class="border w-[829px] h-[158px] border-[#D9D9D9] bg-[#FBFBFB] rounded p-2 mb-4" />
          <div class="flex justify-end">
            <button :class="buttonClass" class="h-[40px] w-[100px] text-center text-white px-4 py-2 rounded mr-2"
      ...
      <div v-if="isEachModalVisible"
        ...
        <div
          ...
          <Form class="flex flex-col gap-4" v-slot='{ submit }' id="reject-form">
            <Textarea v-if="reason === 'Other'" name="discription" v-model="discription" validation="required"
              placeholder="State the reason for rejection"
              class="border w-[829px] h-[158px] border-[#D9D9D9] bg-[#FBFBFB] rounded p-2 mb-4" />
            <Select v-else v-model="reason" name="reason" validation="required"
            ...
            <div class="flex justify-end">
              <button class="bg-[#FF4040] h-[40px] w-[82px] text-white px-4 py-2 rounded mr-2"
      ...
      <div v-if="isModalVisible && selected?.length > 0"
        ...
        <div
          ...
          <select v-model="reason" class="border w-[829px] border-[#D9D9D9] bg-[#FBFBFB] rounded p-2 mb-4">
            <option value="" disabled selected>
              State the reason for rejection
            </option>
          ...
          <!-- Textarea displayed only when "Other" is selected -->
          <textarea v-if="reason === 'Other'" v-model="discription" placeholder="State the reason for rejection"
            class="border w-[829px] h-[158px] border-[#D9D9D9] bg-[#FBFBFB] rounded p-2 mb-4" />
          <div class="flex justify-end">
      ...
      {{ pagination.pending.value }} sdfdf
      <Table :pending="pagination.pending.value" :firstCol="isRoleHrdi" :headers="{
        head: [
          'Ernp ID',
          'Full Name',
          'Gender',
        ...
        <template #actions="{ row }">
          <div v-if="isRoleHrdi && row?.registrationStatus == 'waiting' && row?.academicYear <= '1'" class="flex gap-2">
            ...
            <button @click="confirmeachSelection(row.ernpId)">
              <svg v-if="!sent.includes(row.ernpId)" width="24" height="25" viewBox="0 0 24 25" fill="none"
          ...
          <div v-else-if="isRoleHrdi && row?.registrationStatus == 'transferred' && row?.academicYear <= '1'"
            class="flex gap-2">
            <button @click="showEachModal(row.ernpId)">
              <svg width="18" height="21" viewBox="0 0 18 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
      ...
      :rows="filteredStudents">
        <template #actions="{ row }">
          ...
          <div v-else-if="row?.registrationStatus == 'rejected'">
            ...
            <button class="items-center bg-[#092537] text-white flex gap-2 font-dm-sans p-2 rounded-md"
    ...
    <div v-if="showRejectionReasonModal" class="fixed inset-0  flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white ml-48 rounded-lg shadow-lg gap-3 flex flex-col space-between-[24px] p-6 w-[877px] h-[302px]">
        <div class="flex justify-between">
          <h2 class="text-left flex font-dm-sans leading-[24px] text-[14px] font-bold text-[#4E585F]">
            Reject Reason
          </h2>
    ...
    <div v-if="isModalVisibles"
      class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-4xl relative animate-fade-in-down">
        <!-- Close Button -->
        <button @click="closeEditModal" class="absolute top-3 right-3 text-gray-500 hover:text-gray-700 transition">
        ...
        <form @submit.prevent="saveChanges" class="grid grid-cols-1 gap-y-4 gap-x-6  md:grid-cols-3 md:gap-x-8">
          <div class=" py-4 gap-2 px-4 rounded bg-yellow-50 col-span-3"> <label class="font-bold "> Rejection
              Reason</label>
            <br>{{ selectedRow.rejectionReason }}
          </div>
          <div class=" bg-blue-50 flex col-span-3 py-2 px-4 rounded justify-between ">
            ...
            <div class=" grid flex-1 grid-cols-2 gap-2">
              ...
              <div class="  ">
                ...
                <select v-model="selectedRow.programName" id="salary"
                  class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
                  <option :value="prog.programName" :key="prog.programName" v-for="prog in program || []">
                    {{ prog.programName }}
                  </option>
                </select>
              </div>
          ...
          <div class="col-span-1">
            ...
            <input v-model="selectedRow.ernpId" id="ernpId" disabled
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" />
          </div>

          <div class="col-span-1">
            <label for="fullName" class="block text-sm font-medium text-gray-700" readonly>Full Name</label>
            <input v-model="selectedRow.fullName" id="fullName" disabled
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" />
          </div>
          ...
          <div class="col-span-1">
            ...
            <input v-model="selectedRow.email" id="email"
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
              disabled />
          </div>
          ...
          <div class="col-span-1">
            ...
            <input v-model="selectedRow.phone" id="phone"
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
              disabled />
          </div>
          <div class="col-span-1">
            <label for="gender" class="block text-sm font-medium text-gray-700">Gender</label>
            <select :obj="true" v-model="selectedRow.gender" id="gender"
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
              name="gender" validation="required" disabled>
          ...
          <div class="col-span-1">
            ...
            <input type='date' v-model="selectedRow.dateOfBirth" id="dateOfBirth"
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
              disabled />
          </div>
          ...
          <div class="col-span-1">
            ...
            <input v-model="selectedRow.address" id="address"
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
              disabled />
          </div>
          ...
          <div class="col-span-1">
            ...
            <input v-model="selectedRow.duration" id="duration"
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
              disabled />
          </div>
          <div class="col-span-1">
            <label for="salary" class="block text-sm font-medium text-gray-700">Salary</label>
            <input v-model="selectedRow.salary" id="salary"
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
              disabled />
          </div>
          <div class="col-span-1">
            <label for="salary" class="block text-sm font-medium text-gray-700">Contract Amount</label>
            <input v-model="selectedRow.totalTrainingCost" id="salary"
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
              disabled />
          </div>
          ...
          <div class="col-span-1">
            <div class="col-span-1">
              <label for="registrationStatus" class="block text-sm font-medium text-gray-700" style="display: none;">
                Registration Status
              </label>
              <input v-model="selectedRow.registrationStatus" id="registrationStatus" disabled value="registered"
    ...
    <div v-if="showStudent" class="fixed inset-0  flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white ml-48 rounded-lg shadow-lg gap-3 flex flex-col space-between-[24px] p-6 w-[877px]">
        <div class="flex justify-between">
          <h2 class="text-left flex font-dm-sans leading-[24px] text-[14px] font-bold text-[#4E585F]">
            Student Information
          </h2>
        ...
        <form @submit.prevent="saveChanges" class="grid grid-cols-1 gap-y-4 gap-x-6 md:grid-cols-3 md:gap-x-8">
          ...
          <div class="col-span-1">
            <label for="ernpId" class="block text-sm font-medium text-gray-700">ERNP ID</label>
            <input v-model="currentRow.ernpId" id="ernpId" readonly
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" />
          </div>
          ...
          <div class="col-span-1">
            ...
            <input v-model="currentRow.fullName" id="fullName" readonly
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" />
          </div>
          ...
          <div class="col-span-1">
            <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
            <input v-model="currentRow.email" id="email" readonly
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" />
          </div>
          ...
          <div class="col-span-1">
            <label for="phone" class="block text-sm font-medium text-gray-700">Phone</label>
            <input v-model="currentRow.phone" id="phone" readonly
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" />
          </div>
          ...
          <div class="col-span-1">
            <label for="gender" class="block text-sm font-medium text-gray-700">Gender</label>
            <input v-model="currentRow.gender" id="gender" readonly
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" />
          </div>
          ...
          <div class="col-span-1">
            <label for="dateOfBirth" class="block text-sm font-medium text-gray-700">Date of Birth</label>
            <input v-model="currentRow.dateOfBirth" id="dateOfBirth" readonly
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" />
          </div>
          ...
          <div class="col-span-1">
            <label for="address" class="block text-sm font-medium text-gray-700">Address</label>
            <input v-model="currentRow.address" id="address" readonly
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" />
          </div>
          ...
          <div class="col-span-1">
            <label for="duration" class="block text-sm font-medium text-gray-700">Duration</label>
            <input v-model="currentRow.duration" id="duration" readonly
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" />
          </div>
          ...
          <div class="col-span-1">
            <label for="salary" class="block text-sm font-medium text-gray-700">Salary</label>
            <input v-model="currentRow.salary" id="salary" readonly
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" />
          </div>
          ...
          <div class="col-span-1">
            ...
            <input v-model="currentRow.totalSalary" id="totalSalary" readonly
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" />
          </div>
          ...
          <div class="col-span-1">
            ...
            <input v-model="currentRow.programName" id="programName" readonly
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" />
          </div>
          <div class="col-span-1">
            <label for="programName" class="block text-sm font-medium text-gray-700">Program</label>
            <input v-model="currentRow.campusStatus" id="campusStatus" readonly
              class="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" />
          </div>
          <!-- Action Buttons -->
          <div class="flex justify-end gap-3 pt-4 col-span-3">
            <button type="button" @click="withdrawStudent(currentRow.ernpId)"
...
.Table-header {
  ...
  height: 21px;
  padding-top: 0.5rem;
  /* 2 units in Tailwind are equal to 0.5rem */
