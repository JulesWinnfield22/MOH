...
const pagination = usePagination({
  store: sudents,
  cb: (data, config) =>
    getStudentsByUniId(uniId || auth.auth?.user?.universityProviderUuid,removeUndefined({...data, academicYear: academicYear.value})),
  });
...
function showEachModal(row) {
   selectedRow.value = row;
  // Store the specific `ernpId`
...
function passEachSelection(ernpId) {
  if (request.pending.value) return; // Prevent action if request is pending
  sent.value.push(ernpId)
  request.send(
    () => passStudent([ernpId]), // Pass the row inside an array to maintain the structure
    (res) => {
      sent.value.pop()
      if (res.success) {
...
function undoPassEachSelection(ernpId) {
  if (request.pending.value) return; // Prevent action if request is pending
  sent.value.push(ernpId)
  request.send(
    () => UndopassStudent([ernpId]), // Pass the row inside an array to maintain the structure
    (res) => {
      sent.value.pop()
      if (res.success) {
  ...
  <div class="bg-[#FBFBFB] p-5 ">
    <div class="flex justify-between items-center">
      ...
      <div class="p-4 gap-2.5 flex">
        ...
        >
          Selected {{ selected?.length || 0 }}  Students
        </p>
        ...
        <button
          style="padding: 8px 16px; border-radius: 6px"
          class="mt-1.5 text-center items-center justify-center gap-2 flex h-[40px] w-[229px] font-dm-sans bg-[#36CB56] text-white font-bold rounded hover:bg-[#36CB56]"
          @click="passStudents"
        >
   ...
   <div>
      ...
      <input
    class="rounded-lg border w-[50%] h-10 p-2 focus:outline-none focus:ring-2 focus:ring-[#21618C] placeholder-gray-500"
      ...
      >
        <template #actions="{ row }">
          <div class="flex gap-2"  >
            ...
            <button @click="passEachSelection(row.ernpId)">
              <svg
...
>
  ...
  >
    ...
    <!-- Modal Title -->
    <h3 class="text-2xl font-semibold text-gray-800 mb-4 text-center">Undo Student pass to next year</h3>
    
    <div class="text-center mb-6">
      <div class="inline-block   p-4 rounded-md">
        Are you sure you want to undo <span class="font-bold">{{ selectedRow.fullName }}</span>?
      </div>
    </div>
    ...
    <div class="flex justify-end gap-4 pt-4">
      ...
      <button
        type="button"
        @click="undoPassEachSelection(selectedErnpId)"
