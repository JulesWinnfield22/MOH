<script setup>
  ...
  if(props.value.length) {
    props.value.forEach((val) => {
      selectedPrivileges.value[val.privilege] = props.options?.find(op => op.privilege == val.privilege)?.methods || []
    })
  }
  ...
  function remove(option, emit) {
    selected.value = selected.value.filter(el => el?.privilege != option?.privilege)
    ...
    <InputLayout :class="attrs.class" class="h-auto overflow-visible"  :label="label" :error="error">
      ...
      <div @keydown.enter="open = !open" tabindex="0" @click="open = !open" class="relative w-full flex" :ref="setRef">
        ...
        <div v-else class="p-2 w-full h-full flex-wrap gap-2 flex items-center">
          <div class="h-14 flex-shrink-0 overflow-hidden text-xs flex items-center rounded-md bg-primary/30" :key="selecte" v-for="selecte in selected">
            <div class="p-3 flex flex-col gap-1">
              <p class="text-base">{{ selecte?.privilege }}</p>
              <div class="flex gap-2">
                <div @click="toggleMethod(selecte?.privilege, action, emit)" :class="[selecte?.actions.includes(action) ? 'bg-primary text-white' : 'border border-gray-400']" class="rounded-md px-2 text-xs rounded-m uppercase" :key="action" v-for="action in selectedPrivileges[selecte?.privilege]">
                  {{ action }}
                </div>
              </div>
            </div>
