...
function submit(role) {
  ...
  roleRequest.send(
    () => editRole(props.role?.roleUuid, {...role, roleId: parseInt(role.roleId)}),
    res => {
      if(res.success) {
        res.data.privileges = res.data.privileges.map(el => el.privilegeName + "," + el.privilegeCategory)
        roleStore.update(props.role?.roleUuid, res.data)
        addToast({
          type: 'success',
...
<template>
  <FormParent class="w-[60rem]" :error="roleRequest.error.value" title="Update Role">
    <ActivePrivilagesDataProvider v-slot="{ pending, response }">
      ...
      <div v-else class="h-full flex-1 flex justify-center items-center">
        <h-icon name="fa-spinner" class="animate-spin" scale="1.4" />
      </div>      
