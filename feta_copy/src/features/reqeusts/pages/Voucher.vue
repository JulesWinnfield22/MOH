...
import { getDebitAdvice, getDebitede, getCertificateApprover } from '@/features/reqeusts/api/requestsApi'
import { debitedStore } from '@/features/reqeusts/store/debitedStore'
import DataTable from '../components/DataTable.vue'
import { computed } from 'vue'
import DebitedUsersTable from '../components/DebitedUsersTable.vue';
...
const reqApprovers = useApiRequest()
...
const totalAmount = computed(() => {
  return formatCurrency(
    ((amount - 5) * debitedReq.response.value?.length)
  )
})
...
<template>
  ...
  <div v-if="req.pending.value || debitedReq.pending.value" class="w-full p-2 flex justify-center">
    <FormLoading />
  </div>
  <div v-else id='printable'>
    <div class="page relative h-[1122px] w-[794px] bg-white flex gap-2 flex-col items-center">
      ...
      <div class="h-40"></div>
      <p class="text-4xl font-bold font-serif border-b-2 border-black">Primum Receipt voucher</p>
      <div class=" px-8 flex flex-col gap-2 w-full">
        <div class="w-full flex-col flex items-end justify-end">
          <p><b>Date</b>: {{ formatDate(debitedReq.response.value?.[0]?.paidDate) }}</p>
          <p><b>Voucher Number</b>: {{ debitedReq.response.value?.[0]?.certificatePaymentNumber }}</p>
        </div>
        ...
        <div class="flex flex-col">
          <DataTable
            :headers="['S/N', 'Number of Certificates', 'Unit Price', 'Total Amount(In ETB)']"
  
          >
            <tr>
              <td>1.</td>
              <td class="text-xs">
                {{ debitedReq.response.value?.length }}
              </td>
            ...
            <tr>
              <td colspan="2"></td>
              <td class="py-12 relative">
                <div class="inset-0 absolute flex flex-col h-full justify-end text-xs">
                  <div class="w-full flex flex-col justify-end">
                    <p class=" text-right border-y-2 p-1">Revenue Stamp</p>
                    <p class="text-right p-1 font-semibold">Total Amount Including R/S</p>
                  </div>
                </div>              
              </td>
          ...
          <div class="flex gap-2 text-sm">
            <div class="w-full flex flex-col h-[12rem] justify-center items-center">
              <div class="dash dash-long">
                {{ `${reqApprovers.response.value?.[0]?.debitedByFirstName} ${reqApprovers.response.value?.[0]?.debitedByFatherName}` }}
              </div>
              <p class="font-medium">Approved by</p>
              <p>Approved date <b>{{ secondDateFormat(reqApprovers.response.value?.[0]?.debitedDate) }}</b></p>
            </div>
            <div class="w-full flex flex-col h-[12rem] justify-center items-center">
              <div class="dash dash-long">
                {{ `${reqApprovers.response.value?.[0]?.paidByFirstName} ${reqApprovers.response.value?.[0]?.paidByFatherName}` }}
              </div>
              <p class="font-medium">Paid by</p>
              <p>Paid Date <b>{{ secondDateFormat(reqApprovers.response.value?.[0]?.paidDate) }}</b></p>
            </div>
          </div>
        </div>
      </div>
