...
import { getInsuredAttachementFile } from '@/features/reqeusts/api/requestsApi'
...
if(!cacheStore.cache?.[props.attachment.fileName] && props.attachment?.insuredAttachmentUuid) {
  attachementRequest.send(
    () => getInsuredAttachementFile(props.attachment?.insuredAttachmentUuid, {
      responseType: 'blob'
    }),
    res => {
      console.log(res)
  ...
  console.error('[empty] insuredAttachmentUuid')
}
</script>
<template>
