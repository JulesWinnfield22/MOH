...
//   return [...countriesApi].sort((a, b) => a.name.localeCompare(b.name))
// })
</script>
<template>
  <!-- <SectionTitleLineWithButton :icon="mdiReceiptTextOutline" :title="'Certificate  Reports'" main>
    </SectionTitleLineWithButton> -->
  <div class="mt-8 flow-root">
    <div class="w-full flex flex-row justify-start">
      <div class="sm:col-span-2 m-4">
        <div class="mt-1">
          <input id="beginDate" name="beginDate" v-model="beginDate" type="date" autocomplete="beginDate"
    ...
    <div class="w-full flex items-center flex-row justify-start">
      <div class="sm:col-span-1 m-5 flex flex-col items-start">
        <p class="text-md font-bold whitespace-nowrap">Filter By</p>
      </div>
      <div class="sm:col-span-2 m-4 flex items-center">
        ...
        <select name="branch" id="branch" v-model="branch" placeholder="Select Insurance Branch"
          ...
          <option v-for="insurance in branchReq.response.value" :key="insurance.insuranceUuid" :value="insurance.insuranceUuid"
      ...
      <div class="sm:col-span-2 m-4 flex items-center">
        <label for="sex" class="text-md font-medium mr-2">Agencies</label>
        <select class="truncate block w-full px-4 py-2 rounded-md bg-gray-100 border-1 focus:border-gray-500 focus:bg-white focus:ring-0">
          <option :key="agency.agencyUuid" v-for="agency in agenciesReq.response.value" :value="agency.agencyUuid"
            class="py-1 hover:bg-red-500 hover:text-white">
            {{ agency.agencyName }}
          </option>
        </select>
      ...
      <div class="sm:col-span-2 m-4 flex items-center">
        ...
        <select name="branch" id="branch" v-model="destinationCountry" placeholder="Select Insurance Branch"
          style="inline-size: 200px;"
          class="truncate block w-full px-4 py-2 rounded-md bg-gray-100 border-1 focus:border-gray-500 focus:bg-white focus:ring-0">
        ...
        <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
          <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">
            <table class="min-w-full divide-y divide-gray-300">
              ...
              <tbody class="divide-y divide-gray-200 bg-white" v-if="!branchReq.pending.value">
                <tr v-for="(claim, index) in reports" :key="claim.claimUuid" @click="claimDetailView(claim)"
                  ...
                  <td class="whitespace-nowrap py-2 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6" v-else>
                  ...
                  <td class="whitespace-nowrap px-3 py-2 text-sm text-gray-500">
                    <div class="bg-primary px-2 py-1 w-[50%] text-white text-center cursor-pointer rounded-md">
  ...
  <div v-if="!branchReq.pending.value" class="min-w-full flex items-center mx-auto justify-center mt-50">
    <div class="flex flex-col items-center">
      <div class="round-xl flex gap-0.5 item-center text-black px-3 py-2">
        <div class="text-[#55555550] h-32 w-32">
          <h-icon name="bi-receipt" class="w-full h-full" />
        </div>
      </div>
  ...
  <div v-if="branchReq.pending.value" class="min-w-full p-4 flex items-center mx-auto justify-center">
    <FormLoading />
  </div>
</template>
