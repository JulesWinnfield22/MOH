...
export async function getSavedClaims(query, config = {}) {
  const api = new ApiService()
  const qs = getQueryFormObject(query)
  return api.addAuthenticationHeader().get(`${path}/saved/list${qs}`, config)
}
...
export async function getDeniedClaims(qyery = {}, config = {}) {
  const api = new ApiService()
  const qr = getQueryFormObject(qyery)
  return api.addAuthenticationHeader().get(`${path}/rejected/list${qr}`, config)
}
...
export async function createClaim(id, config) {
  const api = new ApiService()
  return api.addAuthenticationHeader().post(`${servicePath}?insuredPersonUuid=${id}`, {}, config)
}
