...
export const useWithPolicies = defineStore('withPoliciesStore', () => {
  const policies = ref([])
  ...
  function update(id, policy) {
    const idx = policies.value.findIndex(el => el.policyUuid == id)
  ...
  function remove(id) {
    policies.value = policies.value.filter(el => el.policyUuid != id)
  }
